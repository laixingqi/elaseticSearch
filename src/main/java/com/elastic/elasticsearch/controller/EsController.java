...
import com.elastic.elasticsearch.entity.EsModel;
import com.elastic.elasticsearch.entity.EsPage;
import com.elastic.elasticsearch.util.ElasticsearchUtil;
...
public class EsController {
	...
	/**
	 * http://127.0.0.1:8080/es/createIndex 创建索引
	 *
	 * @param request
	 * @param response
	 * @return
	...
	/**
	 * 获取数据 http://127.0.0.1:8080/es/getData?id=2018-04-25%2016:33:44
	 *
	 * @param id
	 * @return
	 */
	...
	public String queryMatchData() {
		...
		List<Map<String, Object>> list = ElasticsearchUtil.searchListData(indexName, esType, boolQuery, 10, "name",
				null, "name");
	...
	public String queryWildcardData() {
		...
		List<Map<String, Object>> list = ElasticsearchUtil.searchListData(indexName, esType, queryBuilder, 10, null,
				null, null);
		return JSONObject.toJSONString(list);
	}
	...
	public String queryIntRangeData() {
		...
		List<Map<String, Object>> list = ElasticsearchUtil.searchListData(indexName, esType, boolQuery, 10, null, null,
				null);
		return JSONObject.toJSONString(list);
	}
	...
	public String queryDateRangeData() {
		BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
		boolQuery
				.must(QueryBuilders.rangeQuery("date").from("2018-04-25T08:33:44.840Z").to("2019-04-25T10:03:08.081Z"));
		List<Map<String, Object>> list = ElasticsearchUtil.searchListData(indexName, esType, boolQuery, 10, null, null,
				null);
		return JSONObject.toJSONString(list);
	}
	 ...
	 * 查询分页
	 *
	 * @param startPage 第几条记录开始 从0开始 第1页
	 *                  ：http://127.0.0.1:8080/es/queryPage?startPage=0&pageSize=2
	 *                  第2页
	...
	public String queryPage(String startPage, String pageSize) {
		if (StringUtils.isNotBlank(startPage) && StringUtils.isNotBlank(pageSize)) {
			BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
			boolQuery.must(
					QueryBuilders.rangeQuery("date").from("2018-04-25T08:33:44.840Z").to("2019-04-25T10:03:08.081Z"));
			EsPage list = ElasticsearchUtil.searchDataPage(indexName, esType, Integer.parseInt(startPage),
					Integer.parseInt(pageSize), boolQuery, null, null, null);
			return JSONObject.toJSONString(list);
		} else {
